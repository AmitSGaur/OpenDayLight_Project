...
# Globals
#o = open("out.txt","w")
baseUrl = 'http://192.168.119.134:8080/controller/nb/v2'
...
def get_all_flow_stats():
    resp, content = h.request('http://192.168.119.134:8080/controller/nb/v2/statistics/default/flow', "GET")
...
def delete_all_flows_node(node):
    url = 'http://192.168.119.134:8080/controller/nb/v2/flowprogrammer/default/node/OF/' + node
    resp, content = h.request(url, "GET")
    allFlows = json.loads(content)
    flows = allFlows['flowConfig']
    # Dump all flows
...
def delete_spec_flow_node(node, flowname):
    del_url = 'http://192.168.119.134:8080/controller/nb/v2/flowprogrammer/default/node/OF/' + node + '/staticFlow/' + flowname
    logging.debug('del_url %s', del_url)
    resp, content = h.request(del_url, "DELETE")
    logging.debug('resp %s content %s', resp, content)
...
def delete_all_flows():
    ...
    for node in nodelist:
        nodeid = node['node']['id']
        url = 'http://192.168.119.134:8080/controller/nb/v2/flowprogrammer/default/node/OF/' + nodeid
        resp, content = h.request(url, "GET")
...
# Get all edges
def get_all_edges():
    resp, content = h.request('http://192.168.119.134:8080/controller/nb/v2/topology/default', "GET")
    ...
    #o.close()
    return edges
    #with open('lol.txt', 'w') as outfile:
...
# Open a file to save edges
#output = open("output.xml","wb")
#s = str(edges)
#output.write( s )
